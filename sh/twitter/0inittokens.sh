...
enter_pin()
{
  ...
  export INIT_PIN_CODE=$(echo -n $pin_code)
}
...
request_init_tokens()
{
  ...
  time_stamp=$(secs)
  c=$(nonce)
  sign=$(echo -en "POST&https%3A%2F%2Fapi.twitter.com%2Foauth%2Frequest_token&%3D%26oauth_callback%3Doob%26oauth_consumer_key%3D$CONSUMER_KEY%26oauth_nonce%3D$c%26oauth_signature_method%3DHMAC-SHA1%26oauth_timestamp%3D$b%26oauth_version%3D1.0" | openssl dgst -sha1 -binary -hmac  "$CONSUMER_SECRET&" | base64)

  reply=$(curl --compressed -s -N 'https://api.twitter.com/oauth/request_token' -H "Authorization: OAuth oauth_consumer_key=\"$CONSUMER_KEY\", oauth_signature_method=\"HMAC-SHA1\", oauth_signature=\"$(stupid_enc $sign)\", oauth_timestamp=\"$b\", oauth_nonce=\"$c\", oauth_version=\"1.0\"" -d '=&oauth_callback=oob')
